...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"panel.modeLight": "轻便",
			"panel.modeFull": "全面",
			"panel.modeLightTip": "轻便模式：大纲只显示官方已加载窗口；要加载更早，请点聊天区官方的「加载更早记录」。点击切换全面模式",
			"panel.modeFullTip": "全面模式：进入即主动加载全部历史，长会话会明显变慢。点击切换轻便模式",
			"panel.collapseTip": "收起大纲面板，恢复对话全宽",
			"panel.expandTip": "展开大纲面板"
		};
		const en = {
			...
			"panel.modeLight": "Light",
			"panel.modeFull": "Full",
			"panel.modeLightTip": "Light mode: the outline only reflects the officially loaded window; load older via the chat's official \"load older\" button. Click to switch to Full mode.",
			"panel.modeFullTip": "Full mode: actively loads the entire history on entry; noticeably slower on long sessions. Click to switch to Light mode.",
			"panel.collapseTip": "Collapse the outline panel and restore full chat width",
			"panel.expandTip": "Expand the outline panel"
		};
		 ...
		 * 单页可达 13 万事件，展开即栈溢出）。
		 */
		const MAX_HISTORY_PAGES = 200; // 每页 50 条消息，上限 1 万条消息（防止大会话分页扫历史拖垮 host）
		...
		/**
		 * 抓取一页历史并抽取精简大纲片段：{ messages, turnEnds, firstSeq, hasMore }。
		 * 大纲是「向上无限滚动、一页一页加载」的——每次只翻一页（50 条消息），
		 * 就地抽取精简字段即弃页，不把事件注入会话 store，也不驻留 chunk 等大事件。
		 * 注意：host 按「消息数」分页，单页可达十几万事件，绝不能展开成大数组。
		 * @param beforeSeq - 本页上界（seq < beforeSeq 的更早消息）；undefined 表示最新页。
		 * @returns 精简片段 + 下一页游标（firstSeq，严格更早）+ 是否还有更早。
		 */
		async function fetchOutlinePage(api, sessionId, beforeSeq) {
			let response;
			try {
				...
				return { messages: [], turnEnds: [], firstSeq: beforeSeq, hasMore: false };
			}
			...
			return {
				messages,
				turnEnds,
				firstSeq: page.length > 0 && page[0].seq !== void 0 ? page[0].seq : beforeSeq,
				hasMore: folded.ok && folded.value.hasMore === true
			};
		}

		/**
		 * 全面模式：从最新往回分页抓取**全部**历史并抽取精简大纲源。
		 * 复用 fetchOutlinePage 逐页抓取；MAX_HISTORY_PAGES 为硬上限。
		 * 注意：长会话逐页解析事件对 host 有明显开销，只应在用户选择全面模式时调用。
		 */
		async function fetchOutlineAll(api, sessionId, report) {
			const messages = [];
			const turnEnds = [];
			let beforeSeq;
			for (let i = 0; i < MAX_HISTORY_PAGES; i++) {
				report && report(i);
				const page = await fetchOutlinePage(api, sessionId, beforeSeq);
				messages.unshift(...page.messages);
				turnEnds.unshift(...page.turnEnds);
				if (!page.hasMore || page.firstSeq === void 0) break;
		...
		/**
		 * 大纲模式：light = 轻便（只反映官方已加载窗口，加载更早走官方按钮，零额外开销）；
		 * full = 全面（进入即主动抓取全部历史，长会话性能影响显著）。localStorage 记忆。
		 */
		const OUTLINE_MODE_KEY = "dsh-chat-outline.mode";
		/** 面板收起状态（IDE 布局下：收起即清让位、恢复对话全宽）。 */
		const OUTLINE_COLLAPSED_KEY = "dsh-chat-outline.collapsed";
		function getOutlineMode() {
			try {
				return window.localStorage.getItem(OUTLINE_MODE_KEY) === "full" ? "full" : "light";
			} catch {
				return "light";
			}
		}
		function setOutlineMode(mode) {
			try {
				window.localStorage.setItem(OUTLINE_MODE_KEY, mode);
			} catch {
				// 忽略：隐私模式等场景
			}
		}
		...
		/** 跳转定位时的翻页大小：一次并进 200 条消息（官方 loadOlder 固定 50/页），
		 *  往返、host 解析与聊天区重渲染次数降到 1/4，跳远距离明显更快。 */
		const JUMP_PAGE_SIZE = 200;

		/**
		 * 以更大的页大小把更早历史并进会话窗口。复刻官方 loadOlder 的窗口推进
		 * 逻辑，但 maxMessages 可调；只用 Session 的公开字段
		 * （history/events/views/baseSeq/hasMore/loadingOlder/conversation/notifier），
		 * 任何不支持的情况返回 false，由调用方回退到官方 loadOlder。
		 * @returns true = 已尝试推进一页（可能无进展）；false = 不可用，请回退。
		 */
		async function loadOlderBig(session, pageSize) {
			if (
				!session ||
			...
			if (session.notifier && typeof session.notifier.markDirty === "function") session.notifier.markDirty();
			try {
				const { result } = await session.history({ beforeSeq: session.baseSeq, maxMessages: pageSize });
				if (!result || result.ok !== true) return false;
				const older = result.value.events;
				if (older.length === 0) {
					session.hasMore = result.value.hasMore;
					session.conversation.prepend([], session.hasMore);
					return true;
				}
				const tail = older[older.length - 1];
				if (tail === void 0 || tail.event.seq + 1 !== session.baseSeq) {
					console.error("[dsh-chat-outline] loadOlderBig page discontinuous");
					session.hasMore = false;
					session.conversation.prepend([], false);
					return true;
				}
				session.events = [...older.map((e) => e.event), ...session.events];
				session.views = [...older.map((e) => e.view), ...session.views];
				session.baseSeq = older[0]?.event.seq ?? session.baseSeq;
				session.hasMore = result.value.hasMore;
				session.conversation.prepend(older.map((e) => ({ event: e.event, view: e.view })), session.hasMore);
				return true;
			} catch (error) {
				console.error("[dsh-chat-outline] loadOlderBig failed:", error);
				return false;
			} finally {
		 ...
		 *  3) 目标不在已加载窗口时循环翻页（大页 200 条，失败回退官方 50 条）；
		...
		async function jumpTo(session, key, getChatTabLabels, report, align) {
			...
			for (let i = 0; i < MAX_PAGES; i++) {
				...
				try {
					const ok = await loadOlderBig(session, JUMP_PAGE_SIZE);
					if (!ok) await session.loadOlder(); // 大页不可用（结构变化/被占用）→ 回退官方逐页
				} catch (error) {
		...
		async function jumpToTrajectory(session, entry, which, getTrajectoryTabLabels, report) {
			...
			// 定位（目标记录未加载时由 jumpTrajectoryRow 自动 loadOlder 翻页）
			const ok = await jumpTrajectoryRow(session, rowKey, targetSeq, nodes, report);
			if (!ok) {
		...
		/** 在轨迹表格中定位某行；目标记录未加载时自动 loadOlder 翻页（同 jumpTo 机制）。 */
		async function jumpTrajectoryRow(session, rowKey, targetSeq, nodes, report) {
			let row = findTrajectoryRow(rowKey);
			let page = 0;
			while (row === null && page < MAX_PAGES) {
				const state = session.getSnapshot();
				if (state.hasMore !== true) break;
			...
			if (row === null) {
				...
				let fresh = [];
				try {
					fresh = session.getSnapshot().views.get("trajectory")?.eventNodes ?? [];
				} catch {
					fresh = nodes || [];
				}
				const seqMin = fresh.length > 0 && Number.isFinite(fresh[0].seq) ? fresh[0].seq : targetSeq;
				const seqMax = fresh.length > 0 && Number.isFinite(fresh[fresh.length - 1].seq) ? fresh[fresh.length - 1].seq : targetSeq;
		 ...
		 *  用扫描法（与官方 pagingAnchor 相同思路）：取**完全位于中线以上**的
		 *  最后一条能映射到大纲条目的 [data-chat-anchor-key] 行（工具调用、错误、
		 *  压缩摘要等无对应条目的行跳过）；没有聊天视图时返回 null。
		 *  注意用 bottom 判据而非 top：跳转把目标滚到视口顶部时，下一条消息
		 *  可能「冒头」到中线以上，用 top 判据会把蓝框抢给更晚的条目。
		...
		function currentAnchorKey(accept) {
			...
			// 只此一种判定：取完全位于中线以上的最后一条可映射到大纲条目的行。
			...
			for (const row of flow.querySelectorAll("[data-chat-anchor-key]")) {
				...
				if (rect.bottom > mid) break; // 该行跨过中线（含其后）：不再作为候选
		 ...
		 * 大纲直接来自会话快照（与聊天区同一数据源）。两种模式（头部按钮切换，
		 * localStorage 记忆）：
		 *  - 轻便（默认）：只显示官方已加载窗口的条目，零额外加载；要更早记录，
		 *    请点聊天区官方的「加载更早记录」按钮扩充窗口；
		 *  - 全面：进入即主动分页抓取全部历史（内存只留精简片段，不注入会话
		 *    store），长会话性能影响显著。
		 */
		function OutlinePanel({ t: tProp, sessions, api, getChatTabLabels, getTrajectoryTabLabels, jumpTo: doJump, jumpToTrajectory: doTrajectoryJump }) {
			...
			// 历史片段状态（全面模式下填充；轻便模式保持为空，只显示官方已加载窗口）
			const [historyMessages, setHistoryMessages] = react.useState([]);
			const [historyTurnEnds, setHistoryTurnEnds] = react.useState([]);
			const [historyLoading, setHistoryLoading] = react.useState(false);
			// 大纲模式（light/full），localStorage 记忆
			const [mode, setMode] = react.useState(() => getOutlineMode());
			const modeRef = react.useRef(mode);
			modeRef.current = mode;
			// 面板收起状态（IDE 布局：收起 = 清让位、恢复对话全宽），localStorage 记忆
			...
			const toggleCollapsed = () => {
				setCollapsed((current) => {
					const next = !current;
					try {
						window.localStorage.setItem(OUTLINE_COLLAPSED_KEY, next ? "1" : "0");
					} catch {
						// 忽略
					}
					return next;
			...
			// 全面模式是否已为当前会话抓取完毕（防重复/防空会话反复抓）
			const fullLoadedRef = react.useRef(false);
			...
			const historyEntries = react.useMemo(
				() => buildOutlineFromSource({ messages: historyMessages, turnEnds: historyTurnEnds }),
				[historyMessages, historyTurnEnds]
			);
			...
			// 全面模式：进入（对话加载完，openState=open）即抓取全部历史构建完整大纲，
			// 不抢对话初始加载资源；轻便模式：不做任何历史抓取，大纲只反映官方已加载窗口。
			const openState = snapshot ? snapshot.openState : null;
			react.useEffect(() => {
				if (mode !== "full" || !panelVisible || session === null) return;
				if (openState !== "open") return; // 等对话加载完再抓
				if (fullLoadedRef.current || historyLoading) return;
				fullLoadedRef.current = true;
				setHistoryLoading(true);
				const sid = sessionId;
				fetchOutlineAll(api, sessionId, () => {})
					.then((source) => {
						if (sid !== lastSessionIdRef.current || modeRef.current !== "full") return;
						setHistoryMessages(source.messages);
						setHistoryTurnEnds(source.turnEnds);
					})
					.catch((error) => {
						console.error("[dsh-chat-outline] full outline fetch failed:", error);
					})
					.finally(() => {
						if (sid === lastSessionIdRef.current) setHistoryLoading(false);
					});
			}, [mode, panelVisible, session, openState, sessionId, api, historyLoading]);
			...
			react.useEffect(() => {
				...
				const measure = () => {
					...
					// IDE 式布局：面板固定 260px 常驻对话区左侧，对话滚动容器 margin-left 让位；
					// 只要放得下「侧边栏 + 面板 + 最小对话宽」就显示（窄窗口也可见）
					const PANEL_W = 260;
					const MIN_CHAT_W = 520;
					const convWidth = frameRect.width - sidebarWidth;
					const visible = convWidth >= PANEL_W + MIN_CHAT_W;
					const height = Math.max(0, frameRect.height - top);
					setGeometry((current) =>
						current &&
						current.left === sidebarWidth &&
						current.top === top &&
						current.width === PANEL_W &&
						current.height === height &&
						current.visible === visible
							? current
							: { left: sidebarWidth, top, width: PANEL_W, height, visible }
					);
			...
			// 让位：面板可见且未收起时，对话滚动容器 margin-left 让出面板宽度
			// （IDE 式布局，VS Code 侧边栏同款）；收起/隐藏时恢复对话全宽
			react.useEffect(() => {
				const el = document.querySelector("[data-conversation-scroll]");
				if (!el) return;
				const apply = geometry !== null && geometry.visible === true && !collapsed && chatActive;
				el.style.marginLeft = apply ? `${geometry.width}px` : "";
			}, [geometry, collapsed, chatActive]);
			...
			if (collapsed) {
				return react.createElement(
					...
					{
						type: "button",
						"data-chat-outline-expand": true,
						title: t("panel.expandTip"),
						onClick: toggleCollapsed,
						style: {
							position: "fixed",
							left: geometry.left + 6,
							top: geometry.top + 10,
							width: 26,
							height: 26,
							zIndex: 1001,
			...
			return react.createElement(
				...
				react.createElement(
					...
					// 模式切换：轻便（默认）/ 全面（进入即加载全部历史，性能开销大）
					react.createElement(
						"button",
						{
							type: "button",
							"data-chat-outline-mode": true,
							title: mode === "light" ? t("panel.modeLightTip") : t("panel.modeFullTip"),
							onClick: () => {
								const next = mode === "light" ? "full" : "light";
								setOutlineMode(next);
								setMode(next);
								if (next === "light") {
									// 切回轻便：清掉已抓取的历史片段，只显示官方已加载窗口
									setHistoryMessages([]);
									setHistoryTurnEnds([]);
									setHistoryLoading(false);
									fullLoadedRef.current = false;
							...
							style: {
								...
								padding: "0 8px",
								borderRadius: 999,
								border: `1px solid ${mode === "full" ? ACCENT : "var(--dsw-alias-border-l2, rgba(0,0,0,.2))"}`,
								background: mode === "full" ? `color-mix(in srgb, ${ACCENT} 14%, transparent)` : "transparent",
								color: mode === "full" ? ACCENT : "var(--dsw-alias-label-caption, #999)",
								cursor: "pointer",
								fontFamily: "inherit",
								whiteSpace: "nowrap"
							}
						},
						mode === "light" ? t("panel.modeLight") : t("panel.modeFull")
					),
					// 收起面板：清让位、恢复对话全宽
					react.createElement(
						"button",
						{
							type: "button",
							"data-chat-outline-collapse": true,
